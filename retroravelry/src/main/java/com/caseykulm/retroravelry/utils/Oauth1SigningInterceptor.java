...
public final class Oauth1SigningInterceptor implements Interceptor {
  ...
  public Request signRequest(Request request) throws IOException {
    String oAuthConsumerKey = getOauthConsumerKey(consumerKey);
    String oAuthNonce = getOauthNonce(random);
    String oAuthTimestamp = getOauthTimestamp(clock);
    String oAuthAccessToken = getOAuthAccessToken(accessToken);
    String oAuthSignature = getOAuthSignature(request, consumerSecret, accessSecret, oAuthConsumerKey, oAuthAccessToken, oAuthNonce, oAuthTimestamp);
    String oAuthSignatureMethod = getOauthSignatureMethod();
    String oAuthVersion = getOAuthVersion();
  ...
  public static String getOAuthVersion() {
    return OAUTH_VERSION_VALUE;
  }

  public static String getOAuthAccessToken(String accessToken) {
    return ESCAPER.escape(accessToken);
  }

  public static String getOauthTimestamp(Clock clock) {
    return clock.millis();
  }

  public static String getOauthSignatureMethod() {
    return OAUTH_SIGNATURE_METHOD_VALUE;
  }

  public static String getOauthConsumerKey(String consumerKey) {
    return ESCAPER.escape(consumerKey);
  }

  public static String getOauthNonce(Random random) {
    byte[] nonce = new byte[32];
    random.nextBytes(nonce);
    return ByteString.of(nonce).base64().replaceAll("\\W", "");
  }

  public static String getOAuthSignature(Request request, String consumerSecret, String accessSecret, String oAuthConsumerKey, String oAuthAccessToken, String oauthNonce, String oauthTimestamp) throws IOException {
    // All OAuth params other than signature added to parameters
    SortedMap<String, String> parameters = new TreeMap<>();
    parameters.put(OAUTH_CONSUMER_KEY, oAuthConsumerKey);
    parameters.put(OAUTH_ACCESS_TOKEN, oAuthAccessToken);
    ...
    // All Request query params escaped and added to parameters
    ...
    // All Request form data parsed and added to parameters
    ...
    // method&url&{paramkey=paramvalue...}
    ...
    return ESCAPER.escape(signature);
  }
